...
const validateSchema = async (schema, filePath) => {
...
async function processSchemas(dir, config, processors = []) {
    if (!config.apis)
        return {};
    ...
    for (const apiConfig of apis) {
        ...
        const processedInputs = await Promise.all(inputFiles.map(async (schema, index) => {
            ...
            const processedFilePath = path.posix.join(dir, `${path.basename(inputPath)}.js`);
...
const viteApiPlugin = async (getConfig) => {
    ...
    const tmpDir = path.posix.join(initialConfig.__meta.rootDir, "node_modules/.zudoku/processed");
    return {
            ...
            if (config.apis) {
                ...
                for (const apiConfig of apis) {
                    if (apiConfig.type === "file" && apiConfig.navigationId) {
                        ...
                        const versionMap = Object.fromEntries(schemas.map((processed) => [
                            processed.version,
                            processed.inputPath,
                        ]));
