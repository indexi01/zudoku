...
const viteDocsPlugin = (getConfig) => {
    ...
    return {
        ...
        watchChange(id, change) {
            if (change.event !== "delete" && change.event !== "create")
                return;
            const config = getConfig();
            const resolver = new DocResolver(config);
            const docsConfigs = resolver.getDocsConfigs();
            const matches = docsConfigs.some((docConfig) => minimatch(ensureLeadingSlash(path.relative(config.__meta.rootDir, id)), docConfig.files));
            if (matches)
                reload(server);
        },
        async load(id) {
            if (id === resolvedVirtualModuleId) {
                ...
                for (let i = 0; i < docsConfigs.length; i++) {
                    ...
                    code.push(`const fileImports${i} = Object.assign({`, ...globbedFiles.map((file) => `  "${ensureLeadingSlash(file)}": () => import("${ensureLeadingSlash(path.posix.join(globImportBasePath, file))}"),`), `});`, `docsPluginOptions.push({`, `  fileImports: fileImports${i},`, `  defaultOptions: ${JSON.stringify(docsConfig.defaultOptions)},`, `  files: ${JSON.stringify(docsConfig.files)}`, `});`);
                }
                ...
                code.push(`const configuredDocsPlugins = [markdownPlugin(docsPluginOptions)]`, `export { configuredDocsPlugins };`);
