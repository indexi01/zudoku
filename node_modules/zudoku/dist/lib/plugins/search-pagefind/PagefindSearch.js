...
export const PagefindSearch = ({ isOpen, onClose, options, }) => {
    ...
    return (_jsxs(CommandDialog, { command: { shouldFilter: false }, content: { className: "max-w-[750px]" }, open: isOpen, onOpenChange: onClose, children: [_jsx(VisuallyHidden, { children: _jsx(DialogTitle, { children: "Search" }) }), _jsx(CommandInput, { ref: inputRef, placeholder: "Search...", value: searchTerm, onValueChange: setSearchTerm, disabled: isError }), _jsx(CommandEmpty, { children: searchTerm ? (_jsxs("div", { className: "flex flex-col items-center", children: ["No results found.", _jsx(Button, { variant: "link", onClick: () => {
                                setSearchTerm("");
                                inputRef.current?.focus();
                            }, children: "Clear search" })] })) : ("Start typing to search") }), isError ? (_jsx("div", { className: "p-4 text-sm", children: error.message === "NOT_BUILT_YET" ? (_jsxs(Callout, { type: "info", children: ["Search is currently not available in development mode by default.", _jsx("br", {}), "To still use search in development, run ", _jsx("code", { children: "zudoku build" }), " ", "and copy the ", _jsx("code", { children: "dist/pagefind" }), " directory to your", " ", _jsx("code", { children: "public" }), " directory."] })) : ("An error occurred while loading search.") })) : (_jsx(ResultList, { searchResults: searchResults ?? [], searchTerm: searchTerm, onClose: onClose, maxSubResults: options.maxSubResults }))] }));
};
